"use client";
import { audiences } from "@/data/metaAdsData";
import MotionWrapper from "./MotionWrapper";
import SectionHeader from "./SectionHeader";

export default function MetaAudience() {
    return (
        <section className="bg-[#f8fbff] px-4 py-16 md:px-8 md:py-24">
            <div className="mx-auto max-w-7xl">
                <SectionHeader title="Who this Meta Ads service is for" />
                        ...
                        return (
                            <MotionWrapper key={item.title} delay={index * 0.12}>
                                <article className="group h-full rounded-[26px] bg-white p-8 shadow-xl shadow-slate-900/5 transition-all duration-300 hover:-translate-y-2 hover:shadow-2xl hover:shadow-blue-500/10">
                                    <div className="mb-5 grid h-14 w-14 place-items-center rounded-2xl bg-[#eaf3ff] text-[#1877f2] transition group-hover:scale-110">
                                        <Icon size={28} />
                                    </div>
