"use client";
import { deliverables } from "@/data/metaAdsData";
    ...
    return (
        <section id="deliverables" className="bg-[#f8fbff] px-4 py-16 md:px-8 md:py-24">
            <div className="mx-auto max-w-7xl">
                <SectionHeader
                    title="What’s included in our Meta Ads service"
                    description="Everything your business needs to launch, manage, and improve paid campaigns."
                />
                        ...
                        return (
                            <MotionWrapper key={item.title} delay={index * 0.12}>
                                <article className="group h-full rounded-[26px] bg-white p-8 shadow-xl shadow-slate-900/5 transition-all duration-300 hover:-translate-y-2 hover:shadow-2xl hover:shadow-blue-500/10">
                                    <div className="mb-5 grid h-14 w-14 place-items-center rounded-2xl bg-[#eaf3ff] text-[#1877f2] transition duration-300 group-hover:scale-110">
                                        <Icon size={28} />
                                    </div>
                                    ...
                                    <ul className="mb-7 grid gap-3">
                                        {item.features.map((feature) => (
                                            <li key={feature} className="flex gap-2 font-semibold text-slate-600">
                                                <span className="font-black text-[#1877f2]">✓</span>
                                                {feature}
                                            </li>
                                        ))}
                                    </ul>
                                    ...
                                    <a
                                        href="#contact"
                                        className="inline-flex w-full justify-center rounded-full bg-[#1877f2] px-6 py-3.5 font-bold text-white transition-all duration-300 hover:-translate-y-1 hover:bg-[#0f5dcc]"
                                    >
                                        {item.button}
                                    </a>
                                </article>
