import { processSteps } from "@/data/tiktokAdsData";
import MotionWrapper from "./MotionWrapper";
import SectionHeader from "./SectionHeader";

export default function TikTokProcess() {
  return (
    <section id="process" className="bg-white px-4 py-16 md:px-8 md:py-24">
      <div className="mx-auto max-w-7xl">
        <SectionHeader
          title="Our TikTok Ads process"
          description="A clear workflow to create engaging ads and improve campaign performance."
        />
          ...
          {processSteps.map((step, index) => (
            <MotionWrapper key={step.title} delay={index * 0.1}>
              <div className="h-full rounded-3xl bg-[#f8fbff] p-7 transition-all duration-300 hover:-translate-y-2 hover:bg-white hover:shadow-xl hover:shadow-slate-900/10">
                <div className="mb-5 grid h-11 w-11 place-items-center rounded-full bg-[#ff0050] text-lg font-extrabold text-white">
                  {index + 1}
                </div>
